...
vi.mock('../MathExpression', () => ({
  MathExpression: ({ expression, fallback }: { expression: string; fallback?: string }) => (
...
describe('TopicCard', () => {
  ...
  it('renders topic information correctly', () => {
    ...
    expect(screen.getByText('Explore variables, equations, and algebraic expressions')).toBeInTheDocument();
  ...
  it('has proper accessibility attributes', () => {
    ...
    expect(card).toHaveAttribute('aria-label', 'Explore Algebra - Explore variables, equations, and algebraic expressions');
    
    const difficultyBadge = screen.getByText('2/5');
    expect(difficultyBadge).toHaveAttribute('aria-label', 'Difficulty level 2 out of 5');
  });

  it('displays correct difficulty colors', () => {
    const easyTopic = { ...mockTopic, difficulty: 1 as const };
    const { rerender } = render(<TopicCard topic={easyTopic} onClick={mockOnClick} />);
  ...
  it('handles topics with multiple prerequisites', () => {
    ...
    render(<TopicCard topic={topicWithMultiplePrereqs} onClick={mockOnClick} />);
  ...
  it('displays correct level labels', () => {
    ...
    levels.forEach(({ level, label }) => {
      ...
      const { rerender } = render(<TopicCard topic={topic} onClick={mockOnClick} />);
  ...
  it('applies custom className', () => {
    const { container } = render(
      <TopicCard topic={mockTopic} onClick={mockOnClick} className="custom-class" />
    );
