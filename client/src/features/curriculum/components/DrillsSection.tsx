import { Printer, RefreshCw, Plus, Minus, X, Divide } from 'lucide-react';
...
export function DrillsSection() {
  ...
  const handlePrint = () => {
    ...
    // Generate the HTML content for the drill worksheet
    const drillHTML = `
      <!DOCTYPE html>
      <html>
        <head>
          <title>Math Farm - ${currentDrillSet.title}</title>
          <style>
            @page {
              size: letter;
              margin: 0.5in;
    ...
    printWindow.document.write(drillHTML);
            ...
            <div key={problem.id} className='drill-problem text-center'>
              ...
              <div className='text-xl font-mono space-y-1'>
                ...
                <div className='border-t-2 border-gray-800 text-right pt-1'>
                  <div className='w-16 h-8 ml-auto'></div>
                </div>
              </div>
            </div>
          ))}
